    ...
    <div class="products-con1 ask-qus-wrap d-flex">
        <div class="chat-wrap">
            <div class="welcome-chat text-center">
                <p>{{__('Welcome to Sortiment LiveChat')}}. {{__('Please choose one of the following questions or write your own')}}</p>
            </div><!-- Welcome chat -->
                ...
                <ul class="chats" id="chat-body">
                    ...
                    @foreach ($chats as $chat)
                        @if ($chat->sender == 'admin')
                            <li class="admin-user">
                                <img src="{{asset($admin->profile_photo_path) }}" class="user-img" alt="" width="50">
                                <span class="msg-cotainer">
                                    @if ($chat->attachment == 1)
                                        <img src="{{ $chat->message }}" alt="chat-attachment" class="chat-attachment">
                                    @else
                                        {{ $chat->message }}
                                    @endif
                        ...
                        @else
                            <li class="employee-user">
                                <img src="{{asset($user->profile_photo_path) }}" class="user-img" alt="" width="50">
                                <span class="msg-cotainer">
                                    @if ($chat->attachment == 1)
                                        <img src="{{ $chat->message }}" alt="chat-attachment" class="chat-attachment">
                                    @else
                    ...
                    {{-- <li>
                        <div class="rating d-flex align-items-center">
                            <div class="col-md-6">
                                <p>Please give Rasmus a rating for this conversation</p>
                            </div>
                            <div class="col-md-6 rating-stars">
            ...
            <div class="chat-foot" id="chat-foot">
                <div action="" class=" d-flex justify-content-between">
                    <input type="text" class="form-control" placeholder="{{__('Write your message here')}}" id="chatInput">
                    <a onclick="sendAttachment()" class="btn btn-blue attach-file"><i class="fas fa-images"></i></a>
                    <button type="button" class="btn btn-blue send-msg-btn" id="send-button">{{__('Send message')}} <i class="fas fa-paper-plane"></i></button>
                </div>
            </div><!-- Chat footer -->
...
<script src="https://cdn.socket.io/4.0.1/socket.io.min.js" integrity="sha384-LzhRnpGmQP+lOvWruF/lgkcqD+WDVt9fU3H4BWmwP5u5LTmkUGafMcpZKNObVMLU" crossorigin="anonymous"></script>
    <script src="{{ asset('backend/js/datetime.js') }}"></script>
    <script type="text/javascript">
        (() => {
            let ip_address = "{{ env('APP_HOST') }}";
            let socket_port = "{{ env('APP_PORT') }}";
            ...
            sendMessage = (message, attachment = 0) => {
                ...
                msg_html += '<img src="{{asset($user->profile_photo_path) }}" class="user-img" alt="" width="50">';
            ...
            socket.on('sendChatToClient', (msgArr) => {
                ...
                for (let c in msgArr) {
                    ...
                    if (chat.user_id == {{ $user->id }} && chat.admin_id == {{ $admin->id }}) {
                        ...
                        msg_html += '<img src="{{asset($admin->profile_photo_path) }}" class="user-img" alt="" width="50">';
